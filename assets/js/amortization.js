(()=>{
 ...
 function updateBreakdown(){
  const chart=$('paymentChart');const monthly=$('chartMonthly');const principalLabel=$('chartPrincipal');const interestLabel=$('chartInterest');
  if(!chart||!monthly||!principalLabel||!interestLabel){return;}
  if(!result||!result.rows||!result.rows.length){
   chart.style.background='conic-gradient(#d7e4f2 0 100%)';
   monthly.textContent='—';principalLabel.textContent='0%';interestLabel.textContent='0%';
   return;
  }
  const first=result.rows[0];const principal=first.principal||0;const interest=first.interest||0;const total=Math.max(principal+interest,1);
  const principalPct=(principal/total)*100;const interestPct=(interest/total)*100;
  chart.style.background=`conic-gradient(#1477ff 0 ${principalPct}%, #1fc98a ${principalPct}% 100%)`;
  monthly.textContent=money(first.payment);
  principalLabel.textContent=`${Math.round(principalPct)}%`;
  interestLabel.textContent=`${Math.round(interestPct)}%`;
 }
 ...
 function calculate(){try{
  ...
  $('payment').textContent=money(result.required);$('payoff').textContent=`Payoff in ${result.months} months · ${dateLabel(start+result.months-1)}`;$('interest').textContent=money(result.interest);$('saved').textContent=money(Math.max(0,normal.interest-result.interest));$('totalPaid').textContent=money(result.total);$('monthsSaved').textContent=String(n-result.months);$('error').textContent='';updateBreakdown();render();
 }catch(e){$('error').textContent=e.message;result=null;$('scheduleBody').innerHTML='';for(const id of ['payment','payoff','interest','saved','totalPaid','monthsSaved'])$(id).textContent='—';updateBreakdown();}}
 let serial=0;
 ...
 $('calculate').onclick=calculate;$('clear').onclick=()=>location.reload();$('scheduleView').onchange=()=>{render();updateBreakdown();};$('addOneTime').onclick=addOneTime;addOneTime();calculate();
})();
